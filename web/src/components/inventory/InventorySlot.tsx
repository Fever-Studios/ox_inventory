  ...
  return (
    <div
      ...
      style={{
        ...
        outline: isOver ? '1px dashed rgba(255,255,255,0.4)' : 'none',
        outlineOffset: '-1px',
      }}
    >
      {/* ── Slot number — top-left badge, always visible for player slots ── */}
      {inventoryType === 'player' && (
        <div className="inventory-slot-number">{item.slot}</div>
      )}

      {/* ── Item image — centered <img>, not backgroundImage ── */}
      {isSlotWithItem(item) && (
      ...
      {/* ── Overlay content — only when slot is populated ── */}
      {isSlotWithItem(item) && (
        <div
        ...
        >
          {/* Count / weight badge — bottom-right glass pill */}
          <div className="item-slot-info-wrapper">
            ...
            {item.count ? <p>{item.count.toLocaleString('en-us')}x</p> : null}
          </div>

          {/* ── Bottom section: durability / price / label ── */}
          <div>
            {/* Durability bar — absolute, pinned to slot bottom */}
                ...
                item.currency ? (
                  ...
                  item.price > 0 && (
                    <div
                      ...
                      style={{ color: item.currency === 'money' || !item.currency ? '#10b981' : '#f43f5e' }}
                    >
                      <p>
                        {Locale.$ || '$'}
            ...
            {/* Item label — gradient overlay at slot bottom */}
